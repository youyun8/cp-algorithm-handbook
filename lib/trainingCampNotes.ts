...
for (int i = 0; i < n; ++i) {
  ...
  'foundation-algorithm-beauty': {
    ...
    implementations: [
      {
        title: '用資料規模反推可用複雜度',
        idea: '賽場上先看 n 的範圍再選演算法：n ≤ 20 可考慮指數；n ≤ 5000 可考慮 O(n^2)；n ≤ 10^5 通常需要 O(n log n)；n ≤ 10^7 多半接近 O(n)。每秒 10^7～10^8 次簡單操作只能作粗估，仍要考慮常數、記憶體與時限。',
...
long long fib(int n) {
    ...
    tips: [
      ...
      '複雜度估算只能看量級：同樣 10^8 次操作會因常數、快取、語言與硬體差很多，不能把它固定換算成 1 秒。'
    ]
  },
...
for (int len = 2; len <= n; ++len) {              // 長度 1 通常在邊界初始化時就設好
    ...
    pitfalls: [
      ...
      '別忘了初始化邊界，尤其是 f[0] 與「無法到達」狀態設成 ±kInf。'
    ],
    tips: [
...
Node* rotate_right(Node* x) {
    ...
    tips: [
      'Treap 的隨機優先級建議用 `std::mt19937` 搭配 `std::random_device` 或固定 seed，避免 `rand()` 的低品質亂數。',
