...
int neg =
    -7 %
    2;  // -1：C++ 的 % 結果符號跟隨被除數（不是除數），與數學上的「模」不同，寫同餘題要特別注意
...
switch (op) {
    case '+':
        ans = a + b;
        break;  // break 讓程式跳出 switch，不會誤跑進 case '-'
    case '-':
        ans = a - b;
        break;
    default:
        ans = 0;  // 沒有任何 case 命中時的兜底分支，習慣放在最後
}`
          }
        ]
      },
...
constexpr int k_max_n = 1'000'000 + 5;  // 多留 5 個緩衝，避免 i+1、i+2 這類下標算式不小心越界
array<int, k_max_n>
    a{};  // {} 觸發值初始化、全部歸零；大小是編譯期常數，配置在靜態記憶體不會讓函式呼叫堆疊爆掉
...
struct Point {
    int x, y;
};  // 把「一個點的兩個座標」打包成一個有語意的型別，勝過維護兩個平行陣列
vector<Point> points(n);
ranges::sort(points, [](const Point& a, const Point& b) {
    return pair{a.x, a.y} <
           pair{b.x, b.y};  // 借用 pair 內建的字典序比較，等同「先比 x，x 相同再比 y」
});`
      },
      {
        title: '指針的應用',
...
struct Node {
    int v;
    Node* next = nullptr;
};  // 指標成員預設初始化成 nullptr，避免宣告後忘記賦值變成「野指標」`
      }
    ]
  },
  {
...
void g(const vector<int>&
           v) { /* 加 const 表示唯讀，既避免複製整個 vector，又防止函式內誤改呼叫端資料 */
}`
          },
          {
            title: '數組參數',
...
void f(span<int>
           a) {  // span 只是「指標 + 長度」的輕量視圖，不複製資料，函式內用 a.size() 就能知道長度
...
array<bool, 100> seen{};  // 另開陣列標記「算過了沒」：因為 fib(n) 的合法回傳值也可能是 0，不能只靠
                          // memo[n] != 0 判斷
long long fib(int n) {
    if (n <= 1) {
        return n;
    ...
    seen[n] = true;
    return memo[n] =
               fib(n - 1) + fib(n - 2);  // 先存進 memo 再回傳，下次遇到同樣的 n 就能 O(1) 命中
}`
          }
        ]
      }
...
void insert(vector<int>& a, int pos, int x) {
    a.insert(a.begin() + pos,
             x);  // 內部會把 pos 之後的元素往後搬一格，本身是 O(n)，但語意比手寫搬移迴圈清楚
}`,
            complexity: 'O(n)'
          },
          {
...
void insert_after(Node* p, int x) {
    p->next =
        new Node{x, p->next};  // 新節點的 next 先接上 p 原本的後繼，再讓 p 指向新節點，順序不能顛倒
}
...
struct Node {
    int val{};
    Node* prev =
        nullptr;  // 多這一個指標讓「從任一節點往回走」變成 O(1)，代價是插入/刪除要多維護一組指標
    Node* next = nullptr;
};`
          },
          {
...
vector<int> val(k_max_n), nxt(k_max_n);  // val[i]/nxt[i] 分別代表「節點 i」的值與下一個節點的下標
int head = -1,
    cnt = 0;  // head 是鏈表起點（-1 代表空表），cnt 是目前已配置的節點數，取代 new/delete`
          }
        ]
      },
      {
...
stack<int> st;
st.push(1);  // 1 現在是棧頂，之後每個 push 的元素都會疊在它上面`
          },
          {
            title: '出棧',
...
tail =
    (tail + 1) %
    k_max_n;  // enqueue：對 kMaxN 取模，讓下標超過陣列尾端時自動繞回開頭，解決順序隊列的「假溢位」
int y = q[head];
head = (head + 1) %
       k_max_n;  // dequeue：同樣取模前進；實務上還需另存 size 或留一格來區分「空」與「滿」`
          }
        ]
      },
      {
...
vector<int> v;
v.push_back(
    3);  // 容量不足時會整批重新配置更大的記憶體並搬移舊資料，但因容量是倍增成長，均攤下來仍是 O(1)
...
q.push(1);
q.front();
q.pop();  // front() 讀頭部的值，pop() 才真正移除它`
          },
          {
            title: 'list（雙向鏈表）',
            summary: '雙向鏈表，任意位置 O(1) 增刪但不支援隨機存取。需要迭代器穩定或頻繁中間增刪時用。',
            code: `// list（雙向鏈表）: 任意位置插入/刪除都是 O(1)（前提是已握有指向該處的迭代器），代價是沒有
// operator[]。
list<int> lst;
lst.push_front(1);
lst.push_back(2);  // 頭尾都能 O(1) 插入，中間插入需先取得迭代器`
          }
        ]
      }
    ]
...
vector<int> lc(k_max_n), rc(k_max_n),
    val(k_max_n);  // 用下標取代指標：lc[i]/rc[i] 存左右孩子的下標，0 代表空，省去動態配置`
          }
        ]
      },
      {
...
void in(Node* r) {
    ...
    in(r->right);
          ...
          },
          {
            ...
            code: `// 後序遍歷: 左 -> 右 ->
// 根，子樹一定比父節點先處理完，適合「先算完子樹資訊再彙總」或「先刪子節點再刪自己」。
void post(Node* r) {
    if (!r) {
        return;
    }
...
while (pq.size() > 1) {  // 只剩一個節點時代表樹已合併完成
    long long a = pq.top();
    pq.pop();  // 貪心：每次都合併「目前最小的兩個」權值
    long long b = pq.top();
    pq.pop();
    cost += a + b;
    pq.push(a + b);  // 合併後的權值要放回堆中，繼續參與後續合併
}`,
        complexity: 'O(n log n)',
        children: [
          {
...
vector<vector<int>> g(
    n + 1,
    vector<int>(n + 1, k_inf));  // 初值設成 kInf（一個夠大的數），代表 u、v 之間預設「不連通」
...
vector<Edge>
    edges;  // 只存邊本身、不記錄「某點的所有鄰居」，正適合這兩種對全體邊逐一排序/鬆弛的算法`
          },
          {
            title: '鄰接表',
...
g[v].push_back({u, w});  // 無向圖的「一條邊」要在兩端都加一次，忘記這行是最常見的圖論 bug`,
          ...
          },
          {
            ...
            code: `// 鏈式前向星: 用陣列模擬鄰接表，靠 head/next 兩個陣列串起每個點的出邊，避免 vector
// 動態配置的額外開銷。
vector<int> head(n + 1, -1), to(k_max_m), nxt(k_max_m);  // head[u] == -1 代表 u 目前沒有任何出邊
int edge_count = 0;
...
int cnt = 0, last = -k_inf;  // last 記錄目前已選區間中最晚的結束時間
for (const auto& it : a) {
    if (it.start >= last) {  // 與已選區間不重疊，才能選它
        ++cnt;
        last = it.end;  // 選了之後，「已佔用到」的時間點更新為這個區間的結束時間
    }
}`,
            complexity: 'O(n log n)'
          },
...
void quick_sort(int l, int r) {
    ...
    static mt19937 rng(
        random_device{}());  // static：整個排序過程只建立一次亂數引擎，避免重複初始化的開銷
    uniform_int_distribution<int> pick(l, r);
    int i = l, j = r, p = a[pick(rng)];  // p 是隨機挑出的基準值（pivot），不直接用 a[l]
                                         // 是為了防止被刻意構造的資料卡效能
    while (i <= j) {
        ...
        children: [
          {
            ...
            code: `// 前綴和與二維前綴和: 把「重複查詢區間和」的成本從每次 O(n) 攤成預處理一次 O(n)、之後每次查詢
// O(1)。 1D prefix sum
for (int i = 1; i <= n; ++i) {
    s[i] =
        s[i - 1] + a[i];  // s[i] 累積 a[1..i] 的總和，下標從 1 開始讓 s[0]=0 能自然代表「空區間」
}
...
d[l] += v;
d[r + 1] -= v;  // 在 l 處 +v 代表「從這裡開始都多 v」，在 r+1 處 -v
                // 代表「從這裡開始抵消掉」，恰好只影響 [l, r]
...
while (i < j) {
    ...
    s < target ? ++i : --j;  // 和太小就需要更大的數，右移左指標；和太大就需要更小的數，左移右指標
          ...
          },
          {
            ...
            code: `// 滑動窗口基礎: 右指標只進不退地擴張視窗，左指標只在條件被破壞時才收縮，兩者合計最多各移動 n
// 次，故整體 O(n)。
int l = 0;
...
x &= ~(1 << k);  // clear the k-th bit to 0：~(1<<k) 是「除了第 k 位都是 1」的遮罩，& 上它會清成
                 // 0、其餘保留
...
bool is_prime(long long n) {
    ...
    for (long long i = 2; i <= n / i;
         ++i) {  // 寫成 i <= n/i 而非 i*i <= n，避免 i*i 在 i 很大時溢位
        if (n % i == 0) {
    ...
    return true;  // 試到 √n 都沒有因子，n 是質數
          ...
          },
          {
            ...
            code: `// 埃氏篩與歐拉線性篩:
// 讓每個合數只被它「最小的質因子」篩掉一次，避免埃氏篩中一個合數被多個質數重複標記。 Euler linear
// sieve
vector<int> primes;
vector<bool> comp(k_max_n);
for (int i = 2; i < k_max_n; ++i) {
    if (!comp[i]) {  // 沒被任何更小的數篩掉過，代表 i 本身是質數
        primes.push_back(i);
    }
    for (int p : primes) {  // 用目前已知的質數（由小到大）去篩 i 的倍數
        if (static_cast<long long>(i) * p >=
            k_max_n) {  // 用 long long 相乘再比較，避免 int 乘法先溢位
            break;
        }
      ...
      },
      {
        ...
        children: [
          ...
          },
          {
            ...
            code: `// 整數快速冪算法: 把指數 b 拆成二進位，a^b = a^(2^0) * a^(2^1) * ...，只在對應位是 1
// 時才乘進答案，共 log(b) 步。
...
vector<int> to_num(const string& s) {
    ...
    for (char ch : views::reverse(s)) {  // 字串是「高位在前」寫的，反著掃就是從個位開始
        a.push_back(ch - '0');  // lowest digit first：這樣 a[0]
                                // 就是個位，之後逐位相加時進位自然往陣列後面（高位）傳播
    }
    return a;
}`
          },
...
bool geq(const vector<int>& a, const vector<int>& b) {
    ...
    for (int i = static_cast<int>(a.size()) - 1; i >= 0;
         --i) {  // 陣列是低位在前，所以從最後一個下標（最高位）開始比
        if (a[i] != b[i]) {
...
vector<int> sub(const vector<int>& a, const vector<int>& b) {
    ...
    for (size_t i = 0; i < a.size();
         ++i) {  // 只需跑到 a 的長度，因為呼叫前已保證 a >= b（b 不會比 a 長）
        int t =
            a[i] - borrow -
            (i < b.size() ? b[i] : 0);  // 扣掉上一輪借的位，再扣掉 b 的對應位（超出 b 長度視為 0）
        borrow = t < 0;  // 不夠減，標記這一位要向高位借 1
        c.push_back((t + 10) %
                    10);  // t 為負時 +10 修正回 0-9 的範圍，非負時 +10 再 %10 等於原值不變
    }
...
vector<int> mul(const vector<int>& a, int b) {
    ...
    for (size_t i = 0; i < a.size() || carry;
         ++i) {  // carry 還沒清完也要繼續，讓進位一路傳到最高位
        if (i < a.size()) {
            carry += static_cast<long long>(a[i]) * b;  // 這一位乘上 b，加上前面傳來的進位
        }
        c.push_back(carry % 10);
        carry /= 10;  // 留下個位當這一位的結果，其餘往下一輪進位
    }
...
vector<int> div(const vector<int>& a, int b, int& rem) {
    ...
    for (int digit : a) {  // 從最高位開始，模擬手算長除法
        int cur =
            rem * 10 + digit;  // 把上一位的餘數乘 10 後併入這一位的數字，湊出這一步真正要除的數
    ...
    return vector<int>(c.begin() + k,
                       c.end());  // 跳過前導零，但至少保留一位（k+1 < size 的條件確保不會刪光）
}`,
            complexity: 'O(len)'
          }
        ]
...
int lower(span<const int> a, int x) {             // first position with value >= x
    int lo = 0, hi = static_cast<int>(a.size());  // 用左閉右開區間 [lo, hi)，hi
                                                  // 從「陣列長度」開始代表「還沒找到就是插在最後」
    while (lo < hi) {
...
int lo = 0, hi = k_max;  // [lo, hi] 是答案可能落在的範圍
while (lo < hi) {
    int mid = lo + (hi - lo) / 2;  // 這種寫法取中點，防止 lo+hi 溢位
    if (check(mid)) {
        hi = mid;  // mid 可行，答案可能就是 mid 或更小，因為不能排除 mid 本身，所以是 hi =
                   // mid（而非 mid - 1）
    } else {
...
for (int i = 0; i < n; ++i) {
    for (int j = capacity; j >= w[i];
         --j) {  // 逆序：此時 f[j - w[i]] 還是「尚未放入第 i 件」的舊值
        f[j] = max(f[j], f[j - w[i]] + v[i]);  // 決策：第 i 件放或不放，取較優者
          ...
          },
          {
            ...
            code: `// 完全背包問題: 與 01 背包唯一差異是容量「正序」遍歷，讓 f[j - w[i]] 可能已包含本輪放過的第 i
// 件，達到「可重複取」的效果。
for (int i = 0; i < n; ++i) {
...
for (int i = 0; i < n; ++i) {
    // 到 i 為止的最佳子段和：要嘛延續前面的子段（cur + a[i]），要嘛從 a[i]
    // 重新開始（放棄收益為負的前綴）
